...
const ARC_CHAIN_CONFIG = { chainId: '0x4cef52', chainName: 'Arc Testnet', nativeCurrency: { name: 'USDC', symbol: 'USDC', decimals: 18 }, rpcUrls: ['https://rpc.blockdaemon.testnet.arc.io', 'https://rpc.drpc.testnet.arc.io'], blockExplorerUrls: ['https://testnet.arcscan.app'] }; 
const CONTRACT_ADDRESS = "0xF83506D10f4416953a6b7CF4cdC5a970CE49B52e"; 
const USDC_ADDRESS     = "0x3600000000000000000000000000000000000000"; 
const ABI_CAFEPAY = ["function registerShop(string memory _shopName) external", "function addItem(string memory _name, uint256 _price) external", "function buyItem(address _shopOwner, uint256 _itemIndex) external", "function getShopMenu(address _shopOwner) external view returns (tuple(uint256 id, string name, uint256 price, bool active)[])", "function shops(address) external view returns (string shopName, address ownerAddress, bool exists)", "function getAllShops() external view returns (address[] memory)" ]; 
const ABI_ERC20 = ["function approve(address spender, uint256 amount) external returns (bool)", "function allowance(address owner, address spender) external view returns (uint256)" ]; 

const firebaseConfig = {
  apiKey: "AIzaSyDnoNpQky_KSlxIaYm2gcn6N6Ykgv3_hQ1o",
  authDomain: "cafepay-f5e37.firebaseapp.com",
  projectId: "cafepay-f5e37",
  storageBucket: "cafepay-f5e37.firebasestorage.app",
  messagingSenderId: "690210290103",
  appId: "1:690210290103:web:ce570b70d386af7582d032",
  measurementId: "G-24LJ632HZ"
};
...
async function getShopFirestoreData(ownerAddr) {
  try {
    const docRef = doc(db, "shops", ownerAddr.toLowerCase());
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      return docSnap.data();
    }
  } catch (err) {
    console.error("Error getting shop Firestore data:", err);
  }
  return {};
}

async function setShopFirestoreData(ownerAddr, data) {
  try {
    const docRef = doc(db, "shops", ownerAddr.toLowerCase());
    await setDoc(docRef, data, { merge: true });
  } catch (err) {
    console.error("Error saving shop Firestore data:", err);
  }
}
...
async function loadShopsDirectory() {
  ...
  if (!grid) return; 
  grid.innerHTML = "<div class='col-span-3 text-center py-20 text-slate-500'>Loading restaurants from blockchain & cloud...</div>"; 
  try {
    ...
    for (const ownerAddr of allShopAddresses) {
      try {
        ...
        if (shop && shop.exists) {
          ...
          const shopData = await getShopFirestoreData(cleanAddr);
          const logoUrl = shopData.logoUrl || null; 
          const customTagline = shopData.tagline || "Fresh food & delicious coffee served daily!"; 
...
async function showCustomerStoreView(shopOwner) {
  ...
  try {
    ...
    const shopData = await getShopFirestoreData(cleanOwner);
    const customTagline = shopData.tagline || "Fresh food & delicious coffee served daily!"; 
    ...
    const itemsData = shopData.items || {};
    menu.forEach((item) => {
      const itemKey = `item_${item.id}`;
      const itemCustom = itemsData[itemKey] || {};
      
      if (itemCustom.deleted === true) return; 
      if (itemCustom.available === false) return; 
      
      const itemName = itemCustom.name || item.name; 
      const basePrice = parseFloat(itemCustom.price !== undefined ? itemCustom.price : ethers.formatUnits(item.price, 6)); 
      const itemDesc = itemCustom.desc || ""; 
      const foodImgUrl = itemCustom.imgUrl; 
...
async function buyCustomItem(shopOwner, itemIndex) {
  ...
  try {
    const shopData = await getShopFirestoreData(shopOwner);
    const itemData = (shopData.items || {})[`item_${itemIndex}`] || {};
    
    const menu = await readOnlyCafePayContract.getShopMenu(shopOwner); 
    const itemObj = menu.find(i => Number(i.id) === Number(itemIndex)); 
    const basePrice = itemData.price !== undefined ? parseFloat(itemData.price) : parseFloat(ethers.formatUnits(itemObj.price, 6)); 
    ...
    const itemName = itemData.name || itemObj.name; 
...
async function updateShopLogo() {
  ...
  try {
    const base64Img = await uploadImageFile('shop-logo-input');
    if (!base64Img) return alert("Please select a valid image file.");
    
    const cleanAddr = ethers.getAddress(userAddress);
    const shopData = await getShopFirestoreData(cleanAddr);
    shopData.logoUrl = base64Img;
    await setShopFirestoreData(cleanAddr, shopData);
    
    alert("Shop logo updated successfully in Cloud!");
...
async function updateShopTagline(cleanAddr) {
  const inputEl = document.getElementById('input-shop-tagline');
  if (!inputEl) return;
  try {
    const shopData = await getShopFirestoreData(cleanAddr);
    shopData.tagline = inputEl.value;
    await setShopFirestoreData(cleanAddr, shopData);
    alert("Shop tagline updated successfully!");
  } catch (err) {
    alert("Error updating tagline: " + err.message);
  }
}
...
async function checkOwnerShopStatus() {
  ...
  try {
    const cleanAddr = ethers.getAddress(userAddress);
    const shop = await cafePayContract.shops(cleanAddr);
...
async function registerShop() {
  ...
  if (!nameInput || !nameInput.value.trim()) return alert("Please enter a shop name.");
  try {
    const tx = await cafePayContract.registerShop(nameInput.value.trim());
    await tx.wait();
    alert("Shop registered successfully on Blockchain!");
...
async function addItem() {
  ...
  if (!nameInput || !priceInput || !nameInput.value.trim() || !priceInput.value) return alert("Please fill item name and price.");
  try {
    const priceVal = ethers.parseUnits(priceInput.value.toString(), 6);
    const tx = await cafePayContract.addItem(nameInput.value.trim(), priceVal);
    await tx.wait();
    alert("Item added successfully to Blockchain!");
    nameInput.value = "";
    priceInput.value = "";
    loadOwnerDashboardMenu();
  } catch (err) {
    alert("Failed to add item: " + (err.reason || err.message));
  }
}
...
async function loadOwnerDashboardMenu() {
  ...
  try {
    ...
    const shopData = await getShopFirestoreData(cleanAddr);
    const currentTagline = shopData.tagline || "Fresh food & delicious coffee served daily!";
    ...
    menu.forEach(item => {
      ...
      const currentName = itemCustom.name || item.name;
      const currentPrice = itemCustom.price !== undefined ? itemCustom.price : ethers.formatUnits(item.price, 6);
      const currentDesc = itemCustom.desc || "";
      const isAvailable = itemCustom.available !== false;
      
      const itemDiv = document.createElement('div');
      itemDiv.className = "bg-slate-900 p-4 rounded-2xl border border-slate-800 flex flex-col gap-3";
      itemDiv.innerHTML = `
        <div class="flex justify-between items-center">
          <span class="font-bold text-white text-sm">#${item.id} - ${currentName}</span>
          <div class="flex gap-2">
            <button onclick="toggleItemAvailability('${cleanAddr}', ${item.id}, ${!isAvailable})" class="text-xs px-3 py-1.5 rounded-lg font-semibold ${isAvailable ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-rose-500/20 text-rose-400 border border-rose-500/30'}">${isAvailable ? 'Available' : 'Hidden'}</button>
            <button onclick="deleteShopItem('${cleanAddr}', ${item.id})" class="text-xs bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 px-3 py-1.5 rounded-lg font-semibold border border-rose-500/30">Delete</button>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-2">
          <input type="text" id="edit-name-${item.id}" value="${currentName}" placeholder="Item Name" class="text-xs bg-slate-800 border border-slate-700 p-2 rounded-xl text-white">
          <input type="number" id="edit-price-${item.id}" value="${currentPrice}" placeholder="Price (USDC)" class="text-xs bg-slate-800 border border-slate-700 p-2 rounded-xl text-white">
        </div>
        <input type="text" id="edit-desc-${item.id}" value="${currentDesc}" placeholder="Description (e.g., Delicious hot coffee)" class="text-xs bg-slate-800 border border-slate-700 p-2 rounded-xl text-white">
        <div class="flex items-center gap-2">
          <input type="file" id="edit-img-${item.id}" class="text-xs text-slate-400 file:mr-2 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-800 file:text-amber-400 hover:file:bg-slate-700">
          <button onclick="updateShopItemDetails('${cleanAddr}', ${item.id})" class="bg-amber-500 hover:bg-amber-600 text-white text-xs px-4 py-2 rounded-xl font-semibold transition ml-auto">Update Item</button>
        </div>
      `;
      dashMenuContainer.appendChild(itemDiv);
    });
  } catch (err) {
...
async function updateShopItemDetails(cleanAddr, itemId) {
  try {
    const nameVal = document.getElementById(`edit-name-${itemId}`).value;
    const priceVal = parseFloat(document.getElementById(`edit-price-${itemId}`).value);
    const descVal = document.getElementById(`edit-desc-${itemId}`).value;
    const base64Img = await uploadImageFile(`edit-img-${itemId}`);
    
    const shopData = await getShopFirestoreData(cleanAddr);
    if (!shopData.items) shopData.items = {};
    const itemKey = `item_${itemId}`;
    
    if (!shopData.items[itemKey]) shopData.items[itemKey] = {};
    shopData.items[itemKey].name = nameVal;
    shopData.items[itemKey].price = priceVal;
    shopData.items[itemKey].desc = descVal;
    if (base64Img) {
      shopData.items[itemKey].imgUrl = base64Img;
    }
...
async function toggleItemAvailability(cleanAddr, itemId, status) {
  try {
    const shopData = await getShopFirestoreData(cleanAddr);
    if (!shopData.items) shopData.items = {};
    const itemKey = `item_${itemId}`;
    if (!shopData.items[itemKey]) shopData.items[itemKey] = {};
    shopData.items[itemKey].available = status;
...
async function deleteShopItem(cleanAddr, itemId) {
  ...
  try {
    ...
    const itemKey = `item_${itemId}`;
    if (!shopData.items[itemKey]) shopData.items[itemKey] = {};
    shopData.items[itemKey].deleted = true;

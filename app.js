...
// ImgBB API দিয়ে ক্লাউডে ছবি আপলোড করার ফাংশন (যাতে অন্য ডিভাইসেও শো করে)
async function uploadImageToCloud(fileInputId) {
    ...
    try {
        // ফ্রি ImgBB API Key ব্যবহার করা হয়েছে
        const response = await fetch(`https://api.imgbb.com/1/upload?key=6d207e02194a847aa964ef4848b94963`, {
            method: "POST",
            body: formData
        });
        const data = await response.json();
        if (data && data.success) {
            return data.data.url; // ক্লাউড ইমেজ লিংক রিটার্ন করবে
        }
    } catch (err) {
...
async function showDirectoryView() {
    // ডিরেক্টরি ভিউতে আসলে ডিসকভার ব্যানার আবার শো করবে
...
async function showCustomerStoreView(shopOwner) {
    // মোবাইল বা ডেক্সটপে শপের ভেতরে গেলে ডিসকভার ব্যানারটি হাইড করে শুধু শপ দেখাবে
    const discoverBanner = document.getElementById('discover-banner-section');
                ...
                <div>
                    ...
                    <div class="mb-4 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <button onclick="adjustQty('${cleanOwner}', ${item.id}, -1, ${basePrice})" class="w-8 h-8 bg-slate-900 border border-slate-700 rounded-xl font-bold text-slate-300 hover:bg-slate-750 transition">-</button>
...
async function updateShopLogo() {
    ...
    const btn = document.getElementById('btn-update-logo');
    if (btn) btn.innerText = "Uploading to Cloud...";

    try {
        const cloudUrl = await uploadImageToCloud('input-shop-logo-file');
        if (!cloudUrl) {
            alert("Please select a valid image file.");
            if (btn) btn.innerText = "Update Logo";
            return;
        }

        localStorage.setItem(`shop_logo_${cleanAddr}`, cloudUrl);
...
async function addItem() {
    ...
    try {
        ...
        // আইটেমের ছবি ক্লাউডে আপলোড করা
        const foodImgUrl = await uploadImageToCloud('new-item-img-file');
        if (foodImgUrl) {
            localStorage.setItem(`item_img_${cleanAddr}_${newItemId}`, foodImgUrl);
        }
